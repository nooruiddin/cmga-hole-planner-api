// api/plan.js — uses Responses API text.format (no response_format)
export default async function handler(req, res) {
  ...
  // Quick ping to verify deploy version
  if (body && body.ping) {
    return res.status(200).json({ ok:true, version:"v2025-08-12-text.format" });
  }
  ...
  // Mock mode (optional): set MOCK=1 in Vercel env to bypass OpenAI
...
`.trim();
  ...
  // ✅ New Responses API shape: text.format (replaces old response_format)
  const payload = {
    model: "gpt-4o-mini",   // if you get “model not found”, change to "o4-mini"
    input: prompt,
    text: {
      format: {
        type: "json_schema",
  ...
  try {
    ...
    if (!outText && Array.isArray(data.output)) {
      outText = data.output.flatMap(o => (o.content||[]).map(c => typeof c.text === "string" ? c.text : "")).join("");
    }
    if (!outText || !outText.trim()) return res.status(200).json({ error: "No text output from model" });

    let out; try { out = JSON.parse(outText); } catch { return res.status(200).json({ error: "Model output invalid JSON", raw: outText.slice(0,2000) }); }
